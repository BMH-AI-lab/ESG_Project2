...
- YOLO26 사용한 이유 
  * 기존 YOLO 계열 모델들은 NMS 기반의 후처리 단계를 포함하고 있어 모델 추론 이후 추가적인 연산과 파라미터 조정이 필요 → 탐지 결과를 후속 분석이나 시스템 연계로 확장하는 과정에서 전체 추론 파이프라인이 복잡해질 수 있음
  * YOLO26은 이러한 후처리 단계에 대한 의존도를 줄인 구조라 모델 출력 이후의 처리 흐름이 보다 단순하고 직관적임 → 따라서, 실시간 객체 탐지 결과를 기반으로 추가 분석이나 외부 시스템과의 연계를 고려한 실험 설계에 유리한 특성을 가짐
