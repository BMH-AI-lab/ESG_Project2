...
- 객체 탐지 모델에는 YOLO26 nano를 사용(YOLOv12 모델 학습도 했음)
  * YOLO 모델은 YOLO26부터는 v1 ~ v12 같은 버전 이름이 안 쓰게 됨 
  * 26은 2026년에 만들었다는 의미이기도 하나 모델의 세대가 바뀌었다는 의미도 가지고 있음
- YOLO26 사용한 이유 
  * 기존 YOLO 계열 모델들은 NMS(Non-Maximum Suppression) 기반의 후처리 단계를 포함하고 있어 모델 추론 이후 추가적인 연산과 파라미터 조정이 필요 → 탐지 결과를 후속 분석이나 시스템 연계로 확장하는 과정에서 전체 추론 파이프라인이 복잡해질 수 있음
  * YOLO26은 이러한 후처리 단계에 대한 의존도를 줄인 구조라 모델 출력 이후의 처리 흐름이 보다 단순하고 직관적임
    * 실시간 객체 탐지 결과를 기반으로 추가 분석이나 외부 시스템과의 연계를 고려한 실험 설계에 유리함
  * 프로젝트에서는 객체 탐지 결과를 기반으로 후속 분석 및 시스템 연계를 고려하고 있어서 후처리 단계가 단순하고 추론 흐름이 명확한 YOLO26 구조가 적합하다고 판단
- 하이퍼 파라미터는 YOLO26에서 주는 기본 파라미터를 사용 
- 학습은 중단된 지점부터 로컬 환경에서 이어서 수행함
  * 초기 학습은 로컬 환경의 메모리 제약으로 인해 Google Colab 환경에서 수행함
    * Colab 환경에서 학습을 진행하며 last.pt 체크포인트에 모델 가중치뿐 아니라 Epoch 정보와 Optimizer 상태가 함께 저장됨
    * 이후 해당 체크포인트를 기반으로 학습을 재개(Resume)함으로써 학습 초기 단계에서 발생하던 메모리 사용 부담이 완화됨 → 이로 인해 중단된 이후 학습 단계는 로컬 환경에서도 정상적으로 수행이 가능했음
...
- 객체 탐지 모델에서 나오는 탐지 결과에 따라 반응
    ...
    - 이상 탐지 관련 태양광 패널이면 LED가 빨간색 불빛을 내며 부저가 울림 
...
- 이미지 탐지 
    ...
    * 업로드를 하면 객체 탐지 모델이 자동으로 태양광 패널의 상태를 탐지
        * 이미지 탐지 하는 동시에 아두이노가 반응
            * 정상 태양광 패널이면 LED가 파랑색 불빛을 냄 
            * 이상 탐지 관련 태양광 패널이면 LED가 빨간색 불빛을 내며 부저가 울림 
        * 이미지 탐지하는 동시에 누적 이미지 창이 새로 생기며 누적 이미지 창 안에 있는 이미지마다 탐지 결과를 보여줌
            * 하나의 이미지 안의 있는 클래스 정확도를 그래프로 수치를 보여줌
            * 하나의 클래스가 여러 개로 탐지시 평균 정확도로 수치를 나타냄
...
- 실시간 탐지 
    * 카메라 웹캠을 통해 실시간으로 탐지 
        * 탐지하는 동시에 탐지 결과가 실시간으로 결과를 보여줌
                * 하나의 이미지 안의 있는 클래스 정확도를 그래프로 수치를 보여줌
                * 하나의 클래스가 여러 개를 탐지시 평균 정확도로 수치를 나타냄
        * 탐지한 결과에 따라 아두이노가 반응
            * 정상 태양광 패널이면 LED가 파랑색 불빛을 냄    
            * 이상 탐지 관련 태양광 패널이면 LED가 빨간색 불빛을 내며 부저가 울림
